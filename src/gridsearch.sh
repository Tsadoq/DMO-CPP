...
if [[ $# -eq 3 ]]; then
    ...
    declare -a Alpha=( "0.3" "0.4" "0.5" "0.6" "0.7" )
    declare -a Cooling=( "0.75" "0.8" "0.85" "0.9" )
    declare -a Mutations=( "2" "3" "4" "5" )
    ...
    for a in ${Alpha[@]}; do  ## param set as array
        ...
        for cool in ${Cooling[@]}; do  ## param set as array
            ...
            for mut in ${Mutations[@]}; do  ## param set as array
                ...
                Files+=($filename)
                ./main $1 $2 4 $a $mut $cool  > ./logs/$1/$filename &
    ...
    echo "Usage: ./gridsearch.sh instance_name runtime num_parallel_instances"
    echo "Example: ./gridsearch.sh instance01 180 2 (2 instances on 4 cores each at a time)"
fi
sleep $2
sleep 3
max_score=99999
for f in ${Files[@]}; do
    if (( $(cat logs/$1/$f | grep -c 'Best solution')  )); then
        echo "[$(date '+%Y-%m-%d_%H:%M:%S')] -- "
        echo "[$(date '+%Y-%m-%d_%H:%M:%S')] -- File: $f"
        score=$(cat logs/$1/$f | grep 'Best solution' | tr -s "\t" | cut -f 2)
        ...
        if [[ ! -d logs/$1/segfault ]];then
            mkdir logs/$1/segfault
        fi  
        mv logs/$1/$f logs/$1/segfault/$f
    fi

done
echo "[$(date '+%Y-%m-%d_%H:%M:%S')] -- Best file found: $best with score $max_score"
if [[ ! -d logs/$1/best ]];then
        mkdir logs/$1/best
fi
mv logs/$1/$best logs/$1/best
# 