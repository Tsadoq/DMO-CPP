...
double Solution::objective_function_unfeasible(int n_exams, int total_number_students, int segna_conflitti){
    double obj_fun=0;
    for(int i=0;i<n_exams;i++){
        obj_fun+=all_exams[i]->weight_in_obj_fun;
    }
    ...
    obj_fun = obj_fun + 10*segna_conflitti;
    return obj_fun;
}